...
export default class GalleryText extends Component {
  state = { active: 0 }
  ...
  componentDidMount() {
    this.props.pictures.map(picture => preloadImage(picture[0]));
  ...
  render({ pictures }, { active }) {
    ...
    const style = {
      backgroundColor: '#444',
      backgroundImage: `url('${pictures[active][0]}')`,
    ...
    return (
      <div>
        ...
        <p style={{ backgroundColor: "#444" }} className="color-white m0 p1">{ pictures[active][1] }</p>
      </div>
    );
  }
}
